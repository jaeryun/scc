...
export async function GET(req: NextRequest) {
  ...
  const op = 'listPlatforms';
  const params = Object.fromEntries(req.nextUrl.searchParams) as Record<string, string>;
  const cacheKey = buildCacheKey('platforms', 'list', params);

  const cached = await checkCache(cacheKey);
  if (cached?.fresh) {
    logger.info(
      { op, cached: true, durationMs: Date.now() - start },
      'Listed platforms (cache hit)'
    );
    return NextResponse.json(success(cached.data));
  }

  try {
    const data = await withRetry(() => netboxAll('/api/dcim/platforms/', params));
    await fetchAndCache(cacheKey, data);
    logger.info({ op, count: data.length, durationMs: Date.now() - start }, 'Listed platforms');
