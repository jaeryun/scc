...
export async function GET(_req: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const dashboard = await getDemoDashboardById(id);
    if (!dashboard) {
      return NextResponse.json(failure('데모 대시보드를 찾을 수 없습니다'), { status: 404 });
    }
    return NextResponse.json(success(dashboard));
  } catch {
    return NextResponse.json(failure('데모 대시보드 조회 실패'), { status: 500 });
  }
}
...
export async function PUT(req: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (body.title !== undefined || body.folderId !== undefined) {
      const existing = await getDemoDashboardById(id);
    ...
    const dashboard = await updateDemoDashboard(id, body);
    if (!dashboard) {
      return NextResponse.json(failure('데모 대시보드를 찾을 수 없습니다'), { status: 404 });
    }
    return NextResponse.json(success(dashboard));
  } catch {
    return NextResponse.json(failure('데모 대시보드 수정 실패'), { status: 500 });
  }
}
...
export async function DELETE(_req: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    const { id } = await params;
    const dashboard = await deleteDemoDashboard(id);
    if (!dashboard) {
      return NextResponse.json(failure('데모 대시보드를 찾을 수 없습니다'), { status: 404 });
    }
    return NextResponse.json(success(dashboard));
  } catch {
    return NextResponse.json(failure('데모 대시보드 삭제 실패'), { status: 500 });
  }
}
