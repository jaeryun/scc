...
export async function GET(req: NextRequest) {
  ...
  if (cached?.fresh) {
    logger.info(
      ...
      'Listed IP addresses (cache hit)'
    );
    return NextResponse.json(success(cached.data));
  }

  try {
    const data = await withRetry(() => netboxAll('/api/ipam/ip-addresses/', params));
    await fetchAndCache(cacheKey, data);
    logger.info({ op, count: data.length, durationMs: Date.now() - start }, 'Listed IP addresses');
